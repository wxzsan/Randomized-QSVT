...
# ==============================================================================
# Step 3: Main - Plot complexity vs overlap (gamma)
# ==============================================================================
if __name__ == "__main__":
    ...
    print(f"ε (eps) = {EPS_FIXED} (fixed accuracy)")
    ...
    for i, molecule in enumerate(molecules):
        ...
        try:
            ...
            total_terms = len(hamiltonian.terms)
        ...
        # --- Use overlap (gamma/eta) as x-axis ---
        overlap_range = np.linspace(0.01, 0.99, 50)
        ...
        for overlap in overlap_range:
            pruning_threshold = EPS_FIXED * overlap * DELTA_VAL
            ...
            complexities["Our Method"].append(our_method_complexity(EPS_FIXED, lambda_val, DELTA_VAL, gamma=overlap))
            complexities["QSVT w/ Trotter"].append(qsvt_trotter_complexity(EPS_FIXED, lambda_val, DELTA_VAL, gamma=overlap, k=K_VAL, L=L))
            complexities["QETU w/ 1st-order Trotter"].append(qetu_first_order_trotter_complexity(EPS_FIXED, lambda_val, DELTA_VAL, gamma=overlap, L=L))
            complexities["QETU w/ Trotter-Suzuki"].append(qetu_trotter_suzuki_complexity(EPS_FIXED, lambda_val, DELTA_VAL, gamma=overlap, k=K_VAL, L=L))
            complexities["QETU w/ qDRIFT"].append(qetu_qdrift_complexity(EPS_FIXED, lambda_val, DELTA_VAL, gamma=overlap))
            complexities["Randomized LCU"].append(other_randomized_methods_complexity(EPS_FIXED, lambda_val, DELTA_VAL, gamma=overlap))
    ...
    # --- Save ---
    fig.savefig(f"chem-gamma-complexity_error={EPS_FIXED}.png", dpi=300, bbox_inches='tight', facecolor='white', edgecolor='none')
